...
$strategicScript = Join-Path $PSScriptRoot "Invoke-CodexStrategicSupervisor.ps1"
$waitScript = Join-Path $PSScriptRoot "Wait-CodexSupervisorState.ps1"
$loopLog = Join-Path $apiRoot "strategic_supervisor_loop.log"
...
while ((Get-Date) -lt $deadline) {
    ...
    powershell -ExecutionPolicy Bypass -NoLogo -NoProfile -File $strategicScript -BindHost $BindHost -Port $Port
